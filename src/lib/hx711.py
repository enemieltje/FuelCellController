...
class HX711(object):
    ...
    def channel_a_gain(self, channel_a_gain):
        if self.channel == "A":
            ...
            logger.warning(
                ...
                """ current channel is '{channel}'""".format(
    ...
    def reset(self):
        ...
        self.power_down()
        logger.debug("power up")
        self.power_up()
        logger.debug("read some raw data")
    ...
    def _validate_channel_name(self, channel):
        ...
        if channel not in self._valid_channels:
            raise ParameterValidationError(
                'channel has to be "A" or "B". I got: ' + str(channel))
    ...
    def _validate_gain_A_value(self, gain_A):
        ...
        if gain_A not in self._valid_gains_for_channel_A:
            raise ParameterValidationError(
                "{gain_A} is not a valid gain".format(gain_A=gain_A))
    ...
    def _set_channel_gain(self, num):
        ...
        for _ in range(num):
            logger.debug("_set_channel_gain called")
    ...
    def _read(self, max_tries=40):
        ...
        for i in range(24):
            ...
            if time_elapsed >= 0.00006:
                logger.debug(
                    'Reading data took longer than 60µs. Time elapsed: {:0.8f}'.format(time_elapsed))
                return False
        ...
        logger.debug('Binary value as it has come: ' + str(bin(data_in)))
    ...
    def get_raw_data(self, times=5):
        ...
        for i in range(times):
            logger.debug(f'raw data {i}/{times}')
            data = self._read()
